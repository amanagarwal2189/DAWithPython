...
def givestats(dictionary, s):
    ...
    print(np.mean(theList))
    print(np.min(theList))
    print(np.max(theList))
    print(np.std(theList))

dict_acct_lesson = {}
for account_key, engagement_records in dict_account_engagements.items():
    total_lessons=0
    for record in engagement_records:
        total_lessons += record['lessons_completed']
    dict_acct_lesson[account_key] = total_lessons

givestats(dict_acct_lesson, "Lessons")
